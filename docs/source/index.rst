...
In order to solve the model numerically, we need values for the parameters.  We will discuss how we use data to choose parameters, but for now we will simply assume :math:`\gamma = 2`, :math:`\beta = 0.99`, :math:`\delta = 0.03`, and :math:`\rho = 0.95`.  We also need a distribution for the innovations and we will assume :math:`\varepsilon \sim N(0,\sigma^2)` where the standard deviation :math:`\sigma = 0.007`.
